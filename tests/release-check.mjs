...
check('live timetable pace follows actual position and remaining clock time', source.includes('function updateScheduleLivePace') && source.includes('var distance=Math.abs(to-pos)') && source.includes('scheduleRequirement(pos,to,left)') && source.includes('расчётный темп'));
