...
check('iPhone foreground return discards stale motion and restarts GPS', source.includes('function restartBrowserGps') && source.includes('gps_resume_restart') && source.includes('rt.speed=0;rt.last=null') && source.includes('iPhone: не блокируй экран — iOS остановит GPS'));check('lightweight telemetry keeps web GPS smooth without rebuilding heavy cards', source.includes('function renderTripTelemetry()') && source.includes('function renderTripAdaptive(force)') && source.includes('rt.posM=currentMeters(); renderTripAdaptive(false); evalAlerts();'));
