...
check('native heat action remains available only to Android', source.includes('thermal-banner platform-android-only') && source.includes('state.settings.screenMode="auto";delete state.settings.wake;persist();syncSw();') && source.includes('Автозатемнение включено, яркость снижена'));
