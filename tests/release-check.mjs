...
check('speed reference uses green main yellow side and red 25 palette', source.includes('🟢 Гл.п — главный путь · 🟡 Бок.п — боковой путь · 🔴 скорость 25 км/ч') && source.includes('.srBadge.glp{border-color:rgba(73,214,132,.38)') && source.includes('.srBadge.bokp{border-color:rgba(245,183,36,.35)') && source.includes('.srBadge.speed25{border-color:rgba(255,67,78,.64)') && source.includes('+rr.glp===25') && source.includes('+rr.bokp===25'));
...
check('all time editors support swipe buttons and half-minutes', source.includes('sap_schedule_overrides') && source.includes('openScheduleTimePicker') && source.includes('function openTimePicker') && source.includes('function bindScheduleDial') && source.includes('id="genericTimeHalf"') && source.includes('data-dial="h"') && source.includes('data-dial="m"') && source.includes('half?":30":""') && source.includes('+a[2]||0') && source.includes('function scheduleRequirement'));
